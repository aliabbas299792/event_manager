...
template<ErrorType Et, typename SetErrorType = ErrorTypeMap<Et>>
ErrorCodes setErrorFromNum(ErrorCodes error, int error_num) {
  constexpr size_t Index = static_cast<size_t>(Et);
  error.emplace<Index>(static_cast<SetErrorType>(error_num));
  return error;
}

ErrorType getErrorType(ErrorCodes &error) {
  return static_cast<ErrorType>(error.index());
}

template<ErrorType Et, typename SetErrorType = ErrorTypeMap<Et>>
std::optional<SetErrorType> getContainedErrorCode(const ErrorCodes &error) {
  constexpr size_t Index = static_cast<size_t>(Et);
