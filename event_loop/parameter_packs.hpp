...
template <> struct RequestToParamPack<RequestType::READ> { using type = ReadParameterPack; };

template <> struct RequestToParamPack<RequestType::WRITE> { using type = WriteParameterPack; };

template <> struct RequestToParamPack<RequestType::CLOSE> { using type = CloseParameterPack; };

template <> struct RequestToParamPack<RequestType::SHUTDOWN> { using type = ShutdownParameterPack; };

template <> struct RequestToParamPack<RequestType::READV> { using type = ReadvParameterPack; };

template <> struct RequestToParamPack<RequestType::WRITEV> { using type = WritevParameterPack; };

template <> struct RequestToParamPack<RequestType::ACCEPT> { using type = AcceptParameterPack; };

template <> struct RequestToParamPack<RequestType::CONNECT> { using type = ConnectParameterPack; };

template <> struct RequestToParamPack<RequestType::OPENAT> { using type = OpenatParameterPack; };

template <> struct RequestToParamPack<RequestType::STATX> { using type = StatxParameterPack; };

template <> struct RequestToParamPack<RequestType::UNLINKAT> { using type = UnlinkatParameterPack; };

template <> struct RequestToParamPack<RequestType::RENAMEAT> { using type = RenameatParameterPack; };
...
struct RequestQueue {
  ...
  void queue_shutdown(int fd, int how) { req_vec.push_back(ShutdownParameterPack{fd, how}); }
