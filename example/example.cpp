...
void example_write_callback(event_manager *ev, processed_data write_metadata, uint64_t pfd) {
  auto length = write_metadata.amount_processed_before + write_metadata.op_res_now;
  std::cout << "Wrote " << length << " bytes of data, the data was: " << std::string(reinterpret_cast<char*>(write_metadata.buff), length) << "\n";
}

void example_read_callback(event_manager *ev, processed_data read_metadata, uint64_t pfd) {
  if(read_metadata.op_res_now > 0) {
    auto length = read_metadata.amount_processed_before + read_metadata.op_res_now;
    std::cout << "Read " << length << " bytes of data, the data was: " << std::string(reinterpret_cast<char*>(read_metadata.buff), length) << "\n";
  }
}
