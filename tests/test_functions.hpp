...
inline void test_accept_callback(event_manager *ev, int listener_fd, sockaddr_storage *user_data, socklen_t size, uint64_t pfd) {
...
inline void test_read_callback(event_manager *ev, processed_data read_metadata, uint64_t pfd) {
...
inline void test_write_callback(event_manager *ev, processed_data write_metadata, uint64_t pfd) {
...
inline void test_shutdown_callback(event_manager *ev, int how, uint64_t pfd) {
  switch (how) {
    case SHUT_RD: {
...
inline void test_close_callback(event_manager *ev, uint64_t pfd) {
  std::cout << "pfd closed: " << pfd << "\n";
}

inline void test_event_callback(event_manager *ev, uint64_t additional_info, int fd) {
  std::cout << "received event signal for: " << fd << ", with additional info: " << additional_info << "\n";
}

inline int connect_to_local_test_server() {
